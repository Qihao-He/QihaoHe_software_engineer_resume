...
\section{Skills}
 \resumeSubHeadingListStart
    ...
    \resumeSubItem{Skills}
      {Git, Embedded system, Shell/scripting, perf, Linux Performance counter, Altera Quartus, LaTeX, power and timing measure, parallel programming, Visual Studio Code, vim}
...
\resumeSubHeadingListStart
    ...
    {Explored the power and energy usage of the GPGPU on the Raspberry Pi and compared with them to the same calculation performed only on their CPUs. Developed 6 benchmarks to do 1D \& 2D FFT with and without GPU rendering using 3 mathematical libraries. The calculation was using single precision floating point numbers. https://csce.ucmss.com/cr/books/2018/AuthorsReport?ConferenceKey=ICM}
    ...
    {Correlated graphics processing, CPU load, power consumption, and total energy. Measured the power consumption difference between GPU rendering and CPU rendering by using different benchmarks both with and without the GPU rendering and an Adafruit USB Power Gauge. https://ieeexplore.ieee.org/document/7881331/}
      
    \resumeSubItem{Henry \& Grace Butler Award Scholarship (2016.08)}
    {Award for the contribution and outstanding in academic and research fields.}
\resumeSubHeadingListEnd
...
\resumeSubHeadingListStart
    ...
    {Applying equations to physical modeling simulation including: projectiles, orbits, Simple Harmonic Oscillator (SHO), Molecules, and using Maxima. Familiar with algebraic manipulation of equations, integration and differentiation of functions, and differential equations. Implementing Euler method, Euler-Cromer method, Euler-Richardson method, Runga-Kutta method to analysis strength and weakness of different methods. Using wxMaxima to correlate dependent equation with coding.}
    ...
    {Built the virtual processor from scratch and downloaded to test board. Five stages have their respective duty: Intruction Fetch (IF), Instruction Decode (ID), Execution (E), Data Memory Access (M), Write Back Stage (WB). Units includes: Forwarding Unit, Hazard Detection Unit, Controls, ALU-Control Unit. Features: mathematical calculation, Read-Write Memory, overcome Data Hazards.}
